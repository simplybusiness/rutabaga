  ...
  describe '.find_feature' do
    ...
    describe "looks for the feature in the spec's directory" do
      ...
      it 'looks directly in the directory' do
        ...
        expect(File).to receive(:exist?).with(%r{spec/rutabaga/different\.feature\Z}).and_return(true)

        expect(subject).to match(%r{spec/rutabaga/different\.feature\Z})
      end

      it 'allows sub-directories' do
        allow(File).to receive(:exist?).with('subdirectory/different.feature').and_return(false)

        @description = 'subdirectory/different.feature'
        expect(File).to receive(:exist?).with(%r{spec/rutabaga/subdirectory/different\.feature\Z}).and_return(true)
    ...
    describe 'figures out the feature name from the spec name' do
      it 'description is nil' do
        @description = nil
        allow(File).to receive(:exist?).with(%r{spec/rutabaga/util\.feature\Z}).and_return(true)
      ...
      it 'description does not match a feature file' do
        ...
        allow(File).to receive(:exist?).with(%r{spec/rutabaga/util\.feature\Z}).and_return(false)
        allow(File).to receive(:exist?).with(%r{spec/rutabaga/util\.rutabaga\Z}).and_return(true)
